...
export default {
  ...
  methods: {
    ...
    /*分页操作------start-------*/
    showCurrentPage() {
      this.$http.get("task/taskHasAccepted/" + this.user.stuid + "/" + this.page.currentPage + "/" + this.page.pageSize).then(resp => {
        console.log(resp);
        const page = this.page;
        const acceptedTasks = resp.data.extend.acceptedTasks
        this.tableData = acceptedTasks.list;
        page.currentPage = acceptedTasks.pageNum;
        page.pageSize = acceptedTasks.pageSize;
        page.total = acceptedTasks.total;
        page.pages = acceptedTasks.pages;
        page.size = acceptedTasks.size;
      })
    },
