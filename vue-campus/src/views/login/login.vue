<template>
  <div class="login-wrap">
    <h1 style="text-align: center">校园互助系统</h1>
    <el-tabs  class="login-container"  v-model="activeName" @tab-click="handleClick" >
      ...
      <el-tab-pane label="登录" name="first">
        ...
        >
          <el-form-item label="学号" prop="studentid" class="elItem">
            <el-input
              type="text"
              autocomplete="off"
          ...
          <el-form-item class="btns">
                          <!--  @submit.native.prevent    和   native-type="submit" 可以实现按enter键登录      -->
            <el-button type="primary" @click="doLogin('loginForm')" native-type="submit">登录</el-button>
            <el-button @click="resetLoginForm('loginForm')">重置</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>
      <el-tab-pane label="注册" name="second">
        <register ref="son" :activeToLogin.sync="activeName"></register>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    handleClick(tab, event) {
      ...
      //重置注册表单
      /*
      父组件调子组件方法 只需在添加的子组件里加入ref
      <register ref="son"></register>
      父组件调用 ：this.$refs.son.子组件方法  即可
      */
      this.$refs.son.resetRegForm();
    },
    //注册
    doLogin(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.get("user/login?studentid="
            +this.loginForm.studentid+"&password="
            ...
            if (resp.data.code===100){
              ...
              this.$router.push({
                path:"/home",
                query:{
                  currentUser:this.loginForm.studentid,
                }
              });
              this.$message({
