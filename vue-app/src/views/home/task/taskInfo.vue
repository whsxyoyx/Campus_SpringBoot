    ...
    methods: {
     ...
     /* getAllTasks(){
      ...
      showCurrentPage() {
        ...
        this.$http.get("admin/getAllTask/"+_this.page.currentPage+"/"+_this.page.pageSize).then(resp=>{

          _this.dates = resp.data.extend.taskList.list;
         //用于显示指定任务包含的图片
          for(let i=0;i< _this.dates.length;i++){
            _this.dates[i].srcList=_this.dates[i].img.split(" ");
            if(_this.dates[i].srcList.length>1){
              _this.dates[i].img=_this.dates[i].srcList[0];
            }
          }



          _this.page.currentPage = resp.data.extend.taskList.pageNum;
          _this.page.pageSize = resp.data.extend.taskList.pageSize;
          _this.page.total = resp.data.extend.taskList.total;
          _this.page.pages = resp.data.extend.taskList.pages;
        })
      },
